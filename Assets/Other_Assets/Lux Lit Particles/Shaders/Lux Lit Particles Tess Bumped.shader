Shader "Lux Lit Particles/ Tess Bumped" {
	Properties {
		...
		[Header(Tessellation)]
		[Space(4)]
		[IntRange] _Tess 			("Subdivisions", Range(1,13)) = 3
	...
	SubShader
	{
		...
		//	As the tesselllation functions rely on ther vert function we include tem afterwards.
			#include "Includes/LuxParticles_Tess.cginc"
			...
			//	Check if we have a directional light
				...
				if (unity_LightPosition[0].w == 0) {
							...
							float atten = GetLightAttenuation(i.worldPos - i.worldViewDir * ( 1.0 - normalSample.b) * _ShadowExtrude); // * i.uv.z );
						#else
