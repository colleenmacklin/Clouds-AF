...
//	SH lighting (from sky)
	...
	#else
//	SH lighting (from probe)
		half3 Lux_ShadeSH (float4 wNormal) {
			half3 x1, x2, x3;
			// Linear + constant polynomial terms
			x1.r = dot(_Lux_L_SHAr, wNormal);
			x1.g = dot(_Lux_L_SHAg, wNormal);
			x1.b = dot(_Lux_L_SHAb, wNormal);
			// 4 of the quadratic polynomials
			half4 vB = wNormal.xyzz * wNormal.yzzx;
			x2.r = dot(_Lux_L_SHBr,vB);
			x2.g = dot(_Lux_L_SHBg,vB);
			x2.b = dot(_Lux_L_SHBb,vB);
			// Final quadratic polynomial
			float vC = wNormal.x*wNormal.x - wNormal.y*wNormal.y;
			x3 = _Lux_L_SHC * vC;
			return (x1 + x2 + x3);
...
//	Per Vertex Lighting function, see: float3 ShadeVertexLightsFull from UniytCG
	inline half3 ApplyVertexLight(float3 viewN, float3 viewpos, float index )
	{
		...
		toLight *= rsqrt(lengthSq);
		// Light Attenuation
		float atten = DistanceAtten(lengthSq, unity_LightAtten[index]) * SpotAtten(toLight, index);
