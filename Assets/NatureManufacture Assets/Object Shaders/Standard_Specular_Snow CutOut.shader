Shader "NatureManufacture Shaders/Standard Shaders/Standard Specular Snow CutOut"
{
	Properties
	{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardSpecular o )
		{
			...
			float3 temp_output_226_0 = appendResult10_g1;
			float3 normalizeResult202 = normalize( BlendNormals( tex2DNode4 , temp_output_226_0 ) );
			...
			float3 normalizeResult201 = normalize( BlendNormals( UnpackScaleNormal( tex2D( _BumpMap, uv0_MainTex ), _SnowNormalCoverHardness ) , temp_output_226_0 ) );
			...
			float3 appendResult169 = (float3(_SnowNormalScale , _SnowNormalScale , 1.0));
			...
			float clampResult225 = clamp( ( tex2D( _SnowMaskB, uv0_SnowMaskB ).b * _SnowMaskPower ) , 0.0 , 1.0 );
			float4 triplanar175 = TriplanarSamplingSF( _SnowHeightG, ase_worldPos, ase_worldNormal, _TriplanarCoverFalloff, temp_output_122_0, 1.0, 0 );
				...
				float staticSwitch215 = ( clampResult225 * saturate( ( ( ( (WorldNormalVector( i , lerpResult115 )).y * clampResult211 ) * ( ( clampResult211 * _SnowHardness ) * temp_output_106_0 ) ) * pow( triplanar175.y , _SnowHeightSharpness ) ) ) );
			#else
				float staticSwitch215 = clampResult225;
			#endif
			#ifdef _USESNOW_ON
				float staticSwitch218 = staticSwitch215;
			#else
				float staticSwitch218 = 1E-05;
			#endif
			float SnowCover220 = staticSwitch218;
			float3 lerpResult177 = lerp( lerpResult193 , lerpResult115 , SnowCover220);
			...
			float4 lerpResult10 = lerp( lerpResult192 , ( triplanar162 * _SnowAlbedoColor ) , SnowCover220);
			...
			float4 lerpResult17 = lerp( ( tex2DNode29 * _SpecularPower ) , float4( ( appendResult151 * _SnowSpecularPower ) , 0.0 ) , SnowCover220);
			o.Specular = lerpResult17.rgb;
			float lerpResult28 = lerp( ( tex2DNode29.a * _SmoothnessPower ) , ( break150.w * _SnowSmoothnessPower ) , SnowCover220);
			...
			float lerpResult27 = lerp( clampResult67 , clampResult69 , SnowCover220);
